...
import { formatEventDateRange, hasRealLocation, isMultiDay } from '../utils';
...
export default function DashboardView({
  ...
  const { isDark, c_cardBg, c_cardBorder, c_cardShadow, c_textSub, t_bg, t_text, t_border } = useAppContext();
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className="w-full mt-6 flex flex-wrap items-center justify-center gap-2">
            {hasRealLocation(heroEvent.location) && (
              <button onClick={() => onToggleMap(heroEvent.id)} className={`inline-flex items-center gap-1.5 text-xs font-semibold ${t_text} ${t_bg} px-4 py-2 rounded-full transition-colors`}><MapPin className="w-3.5 h-3.5" />{expandedMapIds.has(heroEvent.id) ? 'ซ่อนแผนที่' : 'ดูแผนที่'}</button>
            )}
            <button onClick={onShowExport} className={`inline-flex items-center gap-1.5 text-xs font-semibold ${t_text} bg-transparent border ${t_border} px-4 py-2 rounded-full transition-colors hover:${t_bg}`}><ImageIcon className="w-3.5 h-3.5" />ส่งออกภาพ</button>
          </div>
      ...
      {upcomingEvents.length > 0 && (
        <div className="mt-10">
          <div className="flex items-center justify-between mb-4 px-2">
            ...
            <button onClick={() => onTabChange('timeline')} className={`text-xs font-medium ${t_text}`}>ดูทั้งหมด</button>
          </div>
