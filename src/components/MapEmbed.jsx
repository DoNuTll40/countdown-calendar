...
const MapEmbed = React.memo(function MapEmbed({ location, eventId, isExpanded, t_text, c_cardBorder }) {
  ...
  return (
    <AnimatePresence initial={false}>
      {isExpanded && (
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25, ease: 'easeInOut' }} className="overflow-hidden w-full">
          ...
          <div className="mt-2 flex justify-end">
            <a href={getMapsExternalUrl(location)} target="_blank" rel="noopener noreferrer" className={`text-[0.6875rem] font-semibold ${t_text} underline underline-offset-2`}>{L.openInGoogleMaps}</a>
          </div>
        </motion.div>
      )}
    </AnimatePresence>
